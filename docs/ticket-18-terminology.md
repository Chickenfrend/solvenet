...
| Run | One submission and its search policy/state, stored under a `run_id`. All jobs in a run currently target one requested `model`. |
| Initial attempt / initial search chain | One of the independent starting paths requested by the v1 submission field `attempts`. Each starts with one initial job (`repair_depth=0`) and may acquire up to `max_repairs` sequential repair jobs after Lean rejects candidates. Independence means separate job/repair lineage, not a different model, worker, or logical agent. |
...
For submission `attempts = A`, `max_repairs = R`, and `max_assignments = M`,
